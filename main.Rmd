...
# Manhatten Plots of GWAS by GLM and Cor
```{r}
...
Running Blink
```{r}
system("wget http://zzlab.net/GAPIT/data/CROP545_Phenotype.txt")
system("mv CROP545_Phenotype.txt Data")
system("wget http://zzlab.net/GAPIT/data/mdp_numeric.txt")
myCovariate=read.table(file="http://zzlab.net/GAPIT/data/CROP545_Covariates.txt",head=T)

system("/home/john/Documents/ZZClass/BLINK/blink_linux --gwas --file --numeric ")
...
write.table(GD,file="myData.dat",quote=F,sep="\t",col.name=F,row.name=F)
write.table(myPosition,file="myData.map",quote=F,sep="\t",col.name=T,row.name=F)
write.table(data.frame(myPhenotype[,1], simP$y),file="myData.txt",quote=F,sep="\t",col.name=T,row.name=F)
#Run BLINK
system("/home/john/Documents/ZZClass/BLINK/blink_linux --gwas --file myData --numeric")

GMP <- read.delim("Obs_GWAS_result.txt", head = T)
results <- t(as.matrix(GMP[,5]))
plotManhatten(results)
```
